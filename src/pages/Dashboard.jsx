...
function Dashboard({ schoolId }) {
  ...
  async function fetchStats() {
    const { count: lCount } = await supabase.from('learners').select('*', { count: 'exact', head: true }).eq('status', 'active').eq('school_id', schoolId)
    const { count: sCount } = await supabase.from('screener_responses').select('*', { count: 'exact', head: true }).eq('school_id', schoolId)
    const { count: hCount } = await supabase.from('screener_responses').select('*', { count: 'exact', head: true }).eq('risk_level', 'High').eq('school_id', schoolId)
    const { count: pCount } = await supabase.from('learners').select('*', { count: 'exact', head: true }).eq('status', 'pending').eq('school_id', schoolId)
    const { count: stCount } = await supabase.from('staff_registry').select('*', { count: 'exact', head: true }).eq('status', 'active').eq('school_id', schoolId)
    const { data: learners } = await supabase.from('learners').select('seizure_type, triggers').eq('status', 'active').eq('school_id', schoolId)
  ...
  async function resetScreenerCounter() {
    ...
    setResetting(true)
    const { data: rows } = await supabase.from('screener_responses').select('id').eq('school_id', schoolId)
    if (rows && rows.length > 0) await supabase.from('screener_responses').delete().in('id', rows.map(r => r.id))
    await fetchStats()
    setResetting(false)
  ...
  async function deleteAllLearners() {
    ...
    const { data: rows } = await supabase.from('learners').select('id').eq('school_id', schoolId)
    if (rows && rows.length > 0) await supabase.from('learners').delete().in('id', rows.map(r => r.id))
    await fetchStats()
  ...
  async function deleteAllStaff() {
    ...
    const { data: rows } = await supabase.from('staff_registry').select('id').eq('school_id', schoolId)
    if (rows && rows.length > 0) await supabase.from('staff_registry').delete().in('id', rows.map(r => r.id))
    await fetchStats()
  ...
  async function clearDeletedAccountsCache() {
    ...
    let totalRemoved = 0
    try {
      const { data: deletedLearners } = await supabase.from('learners').select('id').eq('status', 'deleted').eq('school_id', schoolId)
      if (deletedLearners?.length > 0) { await supabase.from('learners').delete().in('id', deletedLearners.map(r => r.id)); totalRemoved += deletedLearners.length }
      const { data: deletedStaff } = await supabase.from('staff_registry').select('id').eq('status', 'deleted').eq('school_id', schoolId)
      if (deletedStaff?.length > 0) { await supabase.from('staff_registry').delete().in('id', deletedStaff.map(r => r.id)); totalRemoved += deletedStaff.length }
      await fetchStats()
      alert(`✅ Cache cleared. ${totalRemoved} orphaned record${totalRemoved !== 1 ? 's' : ''} removed.`)
    } catch (err) { alert('⚠️ Error clearing cache. Please try again.') }
  ...
  function BarChart({ data, title, emptyMsg }) {
    ...
    return (
      <div className="card">
        ...
        {data.length === 0 ? <p style={{ color: '#aaa', textAlign: 'center', padding: '20px' }}>{emptyMsg}</p> : (
          data.map((d, i) => (
  ...
  return (
    <div>
      {showReport && <ReportGenerator onClose={() => setShowReport(false)} schoolId={schoolId} />}
      {showReferral && <MedicalReferral onClose={() => setShowReferral(false)} schoolId={schoolId} />}
        ...
        <div className="card" style={{ background: '#fffbe6', border: '1px solid #ffe58f', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ color: '#854F0B', fontSize: '0.8125rem' }}><strong>{pendingCount}</strong> learner{pendingCount > 1 ? 's' : ''} waiting for your review and approval</p>
            </div>
      ...
      <div className="card" style={{ border: '1px solid #ffccc7' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <button className="btn" style={{ background: '#5B21B6', color: 'white' }} onClick={() => setShowReport(true)}>📄 Generate Authority Report</button>
          <button className="btn" style={{ background: '#A32D2D', color: 'white' }} onClick={() => setShowReferral(true)}>⚕️ Medical Referrals</button>
          <button className="btn btn-danger" style={{ background: '#fa8c16' }} onClick={resetScreenerCounter} disabled={resetting}>{resetting ? 'Resetting...' : '🔄 Reset Screener Counter'}</button>
          ...
          <button className="btn btn-danger" style={{ background: '#722ed1' }} onClick={clearDeletedAccountsCache} disabled={clearingCache}>{clearingCache ? 'Clearing...' : '🧹 Clear Deleted Accounts Cache'}</button>
        </div>
      </div>

      <div className="card">
        <h2>🎥 Epilepsy & Seizure First Aid</h2>
        <p style={{ fontSize: '0.8125rem', color: '#666', marginBottom: '12px' }}>A comprehensive video covering the different types of seizures and how to respond with seizure first aid.</p>
        <div style={{ position: 'relative', paddingBottom: '56.25%', height: 0, borderRadius: '12px', overflow: 'hidden' }}>
          <iframe src="https://www.youtube.com/embed/z6K8XQQf9ss" title="Epilepsy and Seizure First Aid" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', border: 'none' }} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
        </div>
      </div>

      <div className="card">
        <h2>About EpiSafe</h2>
        <p style={{ lineHeight: '1.7' }}>EpiSafe helps schools identify learners and staff members at risk of seizures, maintain a secure epilepsy registry, and equip staff with the knowledge to respond effectively.</p>
      </div>
    </div>
  )
}
