...
function Pending({ schoolId }) {
  ...
  async function fetchPending() {
    setLoading(true)
    const { data } = await supabase.from('learners').select('*').eq('status', 'pending').eq('school_id', schoolId).order('created_at', { ascending: false })
    setPending(data || [])
    setSelected(new Set())
    setLoading(false)
  }
  ...
  async function approveSelected() {
    ...
    setApproved(prev => { const next = { ...prev }; approvedLearners.forEach(l => { next[l.id] = l }); return next })
    fetchPending()
  }
  ...
  async function confirmReject() {
    ...
    if (rejectModal === 'bulk') {
      ...
      setRejected(prev => { const next = { ...prev }; rejectedLearners.forEach(l => { next[l.id] = { ...l, reason } }); return next })
    } else {
      await supabase.from('learners').delete().eq('id', rejectModal.id)
  ...
  function sendEmail(learner, subject, message) {
    window.open(`mailto:${learner.emergency_contact_email || ''}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`, '_blank')
  }
    ...
    <div>
      ...
      {selected.size > 0 && (
        <div className="card" style={{ background: '#f0f4f8', border: '1px solid #ddd', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <span style={{ fontSize: '0.875rem', fontWeight: '500', color: '#333' }}>{selected.size} selected</span>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '28px', maxWidth: '500px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize: '0.8125rem', color: '#666', marginBottom: '16px' }}>
              {rejectModal === 'bulk' ? <><strong>{selected.size}</strong> learner(s) selected.</> : <><strong>{rejectModal.full_name}</strong></>}
            </p>
          ...
          {Object.values(approved).map(l => (
            <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(62,207,142,0.2)', flexWrap: 'wrap', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn" style={{ background: '#25D366', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendWhatsApp(l, approvalMessage(l))}>💬 WhatsApp</button>
                <button className="btn" style={{ background: '#4096ff', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendEmail(l, `EpiSafe Registration Approved — ${l.full_name}`, approvalMessage(l))}>📧 Email</button>
                <button className="btn btn-secondary" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => dismissApproved(l.id)}>✖️ Don't send</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ background: '#fff1f0', border: '1px solid #ffccc7', marginBottom: '16px' }}>
          ...
          {Object.values(rejected).map(l => (
            <div key={l.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(255,77,79,0.15)', flexWrap: 'wrap', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <button className="btn" style={{ background: '#25D366', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendWhatsApp(l, rejectionMessage(l, l.reason))}>💬 WhatsApp</button>
                <button className="btn" style={{ background: '#4096ff', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendEmail(l, `EpiSafe Registration Update — ${l.full_name}`, rejectionMessage(l, l.reason))}>📧 Email</button>
                <button className="btn btn-secondary" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => dismissRejected(l.id)}>✖️ Don't send</button>
                <button className="btn btn-danger" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => deleteAccount(l.user_id)}>🗑️ Delete Account</button>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={l.id} className="card" style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px', flex: 1 }}>
                  <input type="checkbox" checked={selected.has(l.id)} onChange={() => toggleSelect(l.id)} style={{ width: 'auto', margin: 0, marginTop: '4px' }} />
                  <div style={{ flex: 1 }}>
