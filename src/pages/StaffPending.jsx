...
function StaffPending({ schoolId }) {
  ...
  async function fetchPending() {
    setLoading(true)
    const { data } = await supabase.from('staff_registry').select('*').eq('status', 'pending').eq('school_id', schoolId).order('created_at', { ascending: false })
    setPending(data || [])
    setSelected(new Set())
    setLoading(false)
  }
  ...
  async function approveSelected() {
    ...
    setApproved(prev => { const next = { ...prev }; approvedStaff.forEach(s => { next[s.id] = s }); return next })
    fetchPending()
  }
  ...
  async function deleteAll() {
    if (!confirm(`Delete ALL ${pending.length} pending record(s)?`)) return
    await supabase.from('staff_registry').delete().in('id', pending.map(s => s.id))
    fetchPending()
  }

  async function deleteAccount(userId) {
    if (!userId) return alert("No account ID found for this record.")
    if (!confirm('This will permanently delete the login account. Continue?')) return
    try {
      const { data, error } = await supabase.functions.invoke('delete-user', { body: { userId, managerCode: 'EPISAFE2025' } })
  ...
  async function confirmReject() {
    ...
    if (rejectModal === 'bulk') {
      ...
      setRejected(prev => { const next = { ...prev }; rejectedStaff.forEach(s => { next[s.id] = { ...s, reason } }); return next })
    } else {
      await supabase.from('staff_registry').delete().eq('id', rejectModal.id)
      setRejected(prev => ({ ...prev, [rejectModal.id]: { ...rejectModal, reason } }))
    }
    setRejectModal(null); setSelectedReason(''); setCustomReason('')
    fetchPending()
  }

  function dismissApproved(id) { setApproved(prev => { const next = { ...prev }; delete next[id]; return next }) }
  function dismissRejected(id) { setRejected(prev => { const next = { ...prev }; delete next[id]; return next }) }

  function sendWhatsApp(s, message) {
    window.open(`https://wa.me/${s.emergency_contact_phone?.replace(/\s/g, '')}?text=${encodeURIComponent(message)}`, '_blank')
  }

  function sendEmail(s, subject, message) {
    window.open(`mailto:${s.emergency_contact_email || ''}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`, '_blank')
  }

  function approvalMessage(s) {
    return `Dear ${s.emergency_contact_name || 'Emergency Contact'},\n\nWe are pleased to inform you that ${s.full_name}'s health registration on EpiSafe School has been APPROVED.\n\nThank you.\nEpiSafe School Health Team`
  }

  function rejectionMessage(s, reason) {
    return `Dear ${s.emergency_contact_name || 'Emergency Contact'},\n\nWe regret to inform you that ${s.full_name}'s health registration could not be approved.\n\nReason: ${reason}\n\nThank you.\nEpiSafe School Health Team`
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px', marginBottom: '8px' }}>
        ...
        {pending.length > 0 && <button className="btn btn-danger" onClick={deleteAll}>🗑️ Delete All ({pending.length})</button>}
      </div>
      ...
      {selected.size > 0 && (
        <div className="card" style={{ background: '#f0f4f8', border: '1px solid #ddd', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          <span style={{ fontSize: '0.875rem', fontWeight: '500' }}>{selected.size} selected</span>
      ...
      {rejectModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', borderRadius: '16px', padding: '28px', maxWidth: '500px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <h2 style={{ marginBottom: '6px' }}>❌ Reject Registration{rejectModal === 'bulk' ? 's' : ''}</h2>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
              <button className="btn btn-secondary" style={{ flex: 1 }} onClick={() => { setRejectModal(null); setSelectedReason(''); setCustomReason('') }}>Cancel</button>
              <button className="btn btn-danger" style={{ flex: 1 }} onClick={confirmReject}>❌ Confirm Reject</button>
            </div>
          </div>
        </div>
      )}
          ...
          {Object.values(approved).map(s => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(62,207,142,0.2)', flexWrap: 'wrap', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button className="btn" style={{ background: '#25D366', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendWhatsApp(s, approvalMessage(s))}>💬 WhatsApp</button>
                <button className="btn" style={{ background: '#4096ff', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendEmail(s, `EpiSafe Registration Approved — ${s.full_name}`, approvalMessage(s))}>📧 Email</button>
                <button className="btn btn-secondary" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => dismissApproved(s.id)}>✖️ Don't send</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="card" style={{ background: '#fff1f0', border: '1px solid #ffccc7', marginBottom: '16px' }}>
          ...
          {Object.values(rejected).map(s => (
            <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid rgba(255,77,79,0.15)', flexWrap: 'wrap', gap: '8px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <button className="btn" style={{ background: '#25D366', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendWhatsApp(s, rejectionMessage(s, s.reason))}>💬 WhatsApp</button>
                <button className="btn" style={{ background: '#4096ff', color: 'white', padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => sendEmail(s, `EpiSafe Registration Update — ${s.full_name}`, rejectionMessage(s, s.reason))}>📧 Email</button>
                <button className="btn btn-secondary" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => dismissRejected(s.id)}>✖️ Don't send</button>
                <button className="btn btn-danger" style={{ padding: '8px 14px', fontSize: '0.75rem' }} onClick={() => deleteAccount(s.user_id)}>🗑️ Delete Account</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {loading ? <p>Loading...</p> : !schoolId ? (
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', padding: '0 4px' }}>
            <input type="checkbox" checked={selected.size === pending.length} onChange={toggleSelectAll} style={{ width: 'auto', margin: 0 }} />
            ...
            <div key={s.id} className="card" style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px', flex: 1 }}>
                  <input type="checkbox" checked={selected.has(s.id)} onChange={() => toggleSelect(s.id)} style={{ width: 'auto', margin: 0, marginTop: '4px' }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                      ...
                      {s.risk_level && (
                        <span style={{ background: riskColor(s.risk_level) + '22', color: riskColor(s.risk_level), border: `1px solid ${riskColor(s.risk_level)}44`, borderRadius: '20px', padding: '2px 10px', fontSize: '0.75rem', fontWeight: '600' }}>
                          {s.risk_level} ({vulnScores[s.risk_level] ?? '—'}/10)
                        </span>
                      )}
                    </div>
