            ...
            <table style={{ minWidth: '700px', width: '100%' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ minWidth: '140px' }}>Seizure Type</th>
                  <th style={{ minWidth: '130px' }}>Vulnerability</th>
                ...
                {filteredLearners.map(l => {
                  ...
                  return (
                    <tr key={l.id}>
                      ...
                      <td>{l.grade}</td>
                      <td style={{ fontSize: '13px' }}>{l.seizure_type || '—'}</td>
                      <td>
                        {vulnLevel ? (
                          <span style={{ background: vulnColors[vulnLevel] + '22', color: vulnColors[vulnLevel], border: `1px solid ${vulnColors[vulnLevel]}44`, borderRadius: '20px', padding: '2px 10px', fontSize: '12px', fontWeight: '600', whiteSpace: 'nowrap' }}>
                            {vulnLevel} ({vulnScores[vulnLevel]}/10)
                          </span>
                        ) : '—'}
                      </td>
                      <td style={{ fontSize: '13px' }}>
                        {l.emergency_contact_name}<br />
                        <small style={{ color: '#888' }}>{l.emergency_contact_phone}</small>
                      </td>
                      <td><span className={badgeClass(l.status)}>{l.status}</span></td>
                      <td>
                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'nowrap' }}>
                          <button className="btn btn-secondary" style={{ padding: '4px 10px', fontSize: '12px', whiteSpace: 'nowrap' }} onClick={() => printLearner(l)}>🖨️ Print</button>
                          <button className="btn btn-danger" style={{ padding: '4px 10px', fontSize: '12px', whiteSpace: 'nowrap' }} onClick={() => deleteLearner(l.id)}>Remove</button>
                        </div>
                      </td>
                    </tr>
                  )
